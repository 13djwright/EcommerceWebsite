<?php
	...
	if(!isset($_SESSION['email']) || empty($_SESSION['email'])) {
		...
		$orderIDstmt = $conn->prepare("SELECT max(id) FROM orders405 where userEmail=?");
		...
		$stmt = $conn->prepare("SELECT id FROM orderDetails405 where orderID=? AND productID=?");
		...
		$stmt->close();
		if($result) {
			$uppstmt = $conn->prepare("UPDATE orderDetails405 D SET D.quantity=? WHERE D.id=?");
			$uppstmt->bind_param("ii",$productQuantity,$result);
			$uppstmt->execute();
			$uppstmt->close();
		} else {
			$addpstmt = $conn->prepare("INSERT INTO orderDetails405(orderID, productID, quantity) VALUES (?,?,?)");
